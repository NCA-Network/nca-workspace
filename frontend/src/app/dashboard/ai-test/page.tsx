...
export default function AITestPage() {
    ...
    chatMutation.mutate(
      { message, conversationId },
      {
        onSuccess: (data) => {
          ...
          setMessages((prev) => [
            ...prev,
            { id: Date.now(), sender: "ai", content: data.response, timestamp: new Date() },
          ]);
        },
        onError: (error) => {
          setMessages((prev) => [
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: Date.now(), sender: "customer", content: userMsg, timestamp: new Date() },
    ]);
    setInput("");
    sendMessage(userMsg);
  };
  ...
  return (
    <div className="p-8 max-w-[900px] mx-auto">
      ...
      >
              ...
              <div className="flex flex-wrap justify-center gap-2">
                {suggestions.map((suggestion) => (
                  <button
                    ...
                    onClick={() => {
                      setMessages([
                        { id: Date.now(), sender: "customer", content: suggestion, timestamp: new Date() },
                      ]);
        ...
        {/* Input Area */}
        <form onSubmit={handleSubmit} className="px-6 py-4 border-t border-[#e0dcd6] flex gap-3">
          <input
            value={input}
