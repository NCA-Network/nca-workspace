...
export default function FAQsPage() {
  ...
  const handleEdit = (faq: NonNullable<typeof faqs>[0]) => {
    setForm({
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (editingId) {
      updateFaq.mutate({ id: editingId, ...data })
    } else {
      createFaq.mutate({
        businessId: 1,
        ...data,
    ...
    <div className="p-8 max-w-[1200px]">
      ...
      {showForm && (
        <form
          ...
          className="bg-white rounded-xl border border-[#e0dcd6] p-6 mb-6"
        >
      ...
      {!faqs || faqs.length === 0 ? (
        ...
        <div className="space-y-3">
          {faqs.map((faq) => (
            <div
              key={faq.id}
              className="bg-white rounded-xl border border-[#e0dcd6] p-5 hover:shadow-sm transition-shadow"
            >
              <div className="flex items-start justify-between gap-4">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-2">
                    <h3 className="font-body text-sm font-medium text-[#1a1814]">
                      {faq.question}
                    </h3>
                    {faq.category && (
                  ...
                  <p className="font-body text-sm text-[#8a8580] leading-relaxed">
                    {faq.answer}
                  </p>
                </div>
