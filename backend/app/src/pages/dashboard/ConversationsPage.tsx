...
export default function ConversationsPage() {
  ...
  const { data: selectedConv } = trpc.conversation.getById.useQuery(
    { id: selectedConvId! },
  ...
  const addMessage = trpc.conversation.addMessage.useMutation({
    onSuccess: () => {
      utils.conversation.getById.invalidate({ id: selectedConvId! })
      utils.conversation.list.invalidate()
      setReplyText('')
    },
  })

  const handoffMutation = trpc.handoff.request.useMutation({
    onSuccess: () => {
      utils.conversation.list.invalidate()
